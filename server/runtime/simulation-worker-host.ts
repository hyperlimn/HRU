...
import type { ObservationCursor, ObservationEventBatch, ObservationFrame, ObservedEntityDetail } from '../../src/observer/observation-types';
...
export class SimulationWorkerHost extends EventEmitter {
  ...
  getObservationFrame(): Promise<ObservationFrame> { return this.request({ type: 'get-observation-frame', requestId: this.id() }); }
  getObservationEvents(cursor?: ObservationCursor, limit?: number): Promise<ObservationEventBatch> { return this.request({ type: 'get-observation-events', requestId: this.id(), ...(cursor ? { cursor } : {}), ...(limit === undefined ? {} : { limit }) }); }
  getObservedEntity(hash: HashHex): Promise<ObservedEntityDetail | undefined> { return this.request({ type: 'get-observed-entity', requestId: this.id(), hash }); }
  ...
  private handle(message: WorkerResponse): void {
    ...
    else if (message.type === 'observation-events') this.emit('observation-events', message.events, message.generation);
