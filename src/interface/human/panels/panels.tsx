...
export function UniversePanel() { const { snapshot } = useRuntime(); return <dl><dt>Universe</dt><dd>{snapshot?.manifest.universeId ?? 'Connecting…'}</dd><dt>Law version</dt><dd>{snapshot?.manifest.lawVersion ?? '—'}</dd></dl>; }
export function TimePanel() {
  const { snapshot, command } = useRuntime();
  ...
  return <div className="stack"><button className="primary" onClick={() => command({ type: 'time/set-running', running: !running })}>{running ? 'Pause' : 'Run'}</button>
    <dl><dt>Current tick</dt><dd>{snapshot?.tick.toLocaleString() ?? '—'}</dd><dt>Actual ticks/sec</dt><dd>{snapshot?.actualTicksPerSecond.toFixed(0) ?? '—'}</dd></dl>
    <label>Requested multiplier<select value={snapshot?.requestedMultiplier ?? 1} onChange={(event) => command({ type: 'time/set-multiplier', multiplier: Number(event.target.value) as Multiplier })}>{MULTIPLIERS.map((value) => <option key={value} value={value}>{value}×</option>)}</select></label></div>;
}
export function ObserverPanel() { return <p>Mouse: orbit, pan, and zoom. Observer state remains local.</p>; }
export function EntityPanel() { return <p>No HRU entities exist yet. Scene points are development markers only.</p>; }
...
export function DimensionsPanel() { const { snapshot } = useRuntime(); return <><p>{snapshot?.activeDimension ?? 'dimension-0'}</p><small>A deterministic lens on the canonical universe.</small></>; }
...
export function SystemPanel() { const { connected, snapshot } = useRuntime(); return <dl><dt>Runtime</dt><dd className={connected ? 'online' : 'offline'}>{connected ? 'Connected' : 'Offline'}</dd><dt>Render FPS</dt><dd><RenderFps /></dd><dt>Modules</dt><dd>{snapshot?.manifest.enabledDeterministicModules.length ?? 0} deterministic</dd></dl>; }
