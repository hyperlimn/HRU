...
import {selectiveBloomIntensity,vfxTargetMatches} from '../vfx/postprocessing/selective-bloom';
...
import {paletteChannelColor} from '../../visual-lab/palettes';
...
export class ThreeObservationRenderer {
  ...
  private color(role:Parameters<typeof paletteChannelColor>[1],identity:string|undefined,explicit:string,channel=role.toLowerCase().replaceAll(' ','-')):string {return paletteChannelColor(this.visual,role,identity??'observer-default',channel,explicit)}
  ...
  constructor(private readonly scene: THREE.Scene) {
    ...
    this.particleField=new ParticleFieldRenderer(this.groups['phase-effects']);
  ...
  setVisualConfiguration(values: VisualConfiguration): { geometryRebuilt: boolean } {
    ...
    const prior=this.visual;this.visual = values;this.particleField.configure(values);this.vortexField.configure(values); if (geometryRebuilt) { this.geometryRevision += 1;this.entityGeometries.forEach((geometry)=>geometry.dispose());this.entityGeometries.clear(); } if (gridChanged) this.rebuildGrid();
  ...
  refreshPalette():void{this.rebuildGrid();this.rebuildCurrent()}
  ...
  update(frame: ObservationFrame, events: readonly VisualEvent[]): void {
    ...
    this.particleField.update(frame,events,this.selected);this.vortexField.update(frame,events,this.selected);for (const visual of events) if (!this.seenEvents.has(visual.event.eventId) && this.addTransient(visual.event, visual.observedAt)) this.seenEvents.add(visual.event.eventId);
  ...
  setSelection(hash?: HashHex): void { this.selected = hash;this.particleField.setSelection(hash);this.vortexField.setSelection(hash); this.rebuildSelection(); }
  particleTelemetry(){return this.particleField.telemetry()}
  particleExplanation(hash:string){return this.particleField.explanation(hash)}
  vortexTelemetry(){return this.vortexField.telemetry()}
  vortexExplanation(hash:string){return this.vortexField.explanation(hash)}
  ...
  dispose(): void {this.particleField.dispose();this.vortexField.dispose(); for (const group of Object.values(this.groups)) { clearGroup(group); this.scene.remove(group); } this.entityGeometries.forEach((geometry)=>geometry.dispose());this.entityGeometries.clear(); }
  ...
  private rebuildCurrent(): void {
    ...
    for (let variation = 0; variation < 3; variation += 1) {
      for(let detail=0;detail<=5;detail+=1){const candidates=this.frame.entities.filter((entity)=>{const traits=renderTraits(entity.hash,entity.provenance,entity.contextHash,Boolean(entity.clusterHash));return traits.geometryVariation===variation&&entityGeometryDetail(entity,this.visual)===detail});const intensityGroups=new Map<number,typeof candidates>();for(const entity of candidates){const target=stringValue(this.visual,'vfx.selective.target');const intensity=vfxTargetMatches(target,'Entities')?selectiveBloomIntensity(this.visual,{entityHash:entity.hash,contextHash:entity.contextHash,dimension:'dimension-0'}):0;const bucket=Math.round(intensity*20)/20;const group=intensityGroups.get(bucket)??[];group.push(entity);intensityGroups.set(bucket,group)}for(const [vfxIntensity,entities] of intensityGroups){if(!entities.length)continue;const key=`${variation}:${detail}`;let geometry=this.entityGeometries.get(key);if(!geometry){geometry=variation===0?new THREE.IcosahedronGeometry(1,detail):variation===1?new THREE.OctahedronGeometry(1,detail):new THREE.DodecahedronGeometry(1,detail);this.entityGeometries.set(key,geometry)}
        const firstTraits=renderTraits(entities[0]!.hash,entities[0]!.provenance,entities[0]!.contextHash,Boolean(entities[0]!.clusterHash));const legacyEmissive=new THREE.Color(0x061824).lerp(new THREE.Color().setHSL(firstTraits.emissiveHue,.8,.5),numberValue(this.visual,'entity.emissiveInfluence'));const emissive=new THREE.Color(this.color('Glow',entities[0]!.hash,`#${legacyEmissive.getHexString()}`,'entity/glow'));const material = new THREE.MeshStandardMaterial({ vertexColors: true, roughness: numberValue(this.visual,'entity.roughness'), metalness: numberValue(this.visual,'entity.metalness'), opacity: numberValue(this.visual,'entity.opacity'), transparent: numberValue(this.visual,'entity.opacity') < 1, emissive, emissiveIntensity: numberValue(this.visual,'entity.emissiveMultiplier')+vfxIntensity,wireframe:booleanValue(this.visual,'entity.wireframe'),depthTest:booleanValue(this.visual,'entity.depthTest'),depthWrite:booleanValue(this.visual,'entity.depthWrite'),toneMapped:vfxIntensity===0 });
    ...
    for (const bond of this.frame.bonds) {
      ...
      const actualTarget=positive?(active?'Positive Bonds':'Weak Bonds'):(active?'Repulsion':'Weak Bonds');const vfxIntensity=vfxTargetMatches(stringValue(this.visual,'vfx.selective.target'),actualTarget)?selectiveBloomIntensity(this.visual,{pairHash:bond.pairHash,bondStrength:bond.strength,dimension:'dimension-0'}):0;const role=positive?(active?'Positive Relationship':'Weak Relationship'):'Repulsion';const bondColor=new THREE.Color(this.color(role,bond.pairHash,stringValue(this.visual,positive?'relationship.positiveColor':'relationship.negativeColor'),'relationship/base')).multiplyScalar(1+vfxIntensity);const material = new THREE.MeshBasicMaterial({ color: bondColor, transparent: true, opacity: (active ? numberValue(this.visual,positive?'relationship.activeOpacity':'relationship.repulsionOpacity') : numberValue(this.visual,'relationship.weakOpacity')), blending: THREE.AdditiveBlending, depthWrite: false,depthTest:booleanValue(this.visual,'relationship.depthTest'),toneMapped:vfxIntensity===0 });
    ...
    for (const cluster of this.frame.clusters) {
      ...
      const segments=numberValue(this.visual,'cluster.segments');const vfxIntensity=vfxTargetMatches(stringValue(this.visual,'vfx.selective.target'),'Clusters')?selectiveBloomIntensity(this.visual,{clusterSize:cluster.memberHashes.length,entityHash:cluster.clusterHash,dimension:'dimension-0'}):0;const clusterMesh=new THREE.Mesh(new THREE.SphereGeometry(radius, segments, Math.max(3,Math.floor(segments*.625))), new THREE.MeshBasicMaterial({ color: new THREE.Color(this.color('Cluster',cluster.clusterHash,stringValue(this.visual,'cluster.color'),'cluster/shell')).multiplyScalar(1+vfxIntensity), wireframe: booleanValue(this.visual,'cluster.wireframe'), transparent: true, opacity: numberValue(this.visual,'cluster.opacity'),toneMapped:vfxIntensity===0 }));clusterMesh.visible=booleanValue(this.visual,'cluster.enabled');this.groups.clusters.add(clusterMesh);
      this.groups.clusters.children.at(-1)!.position.copy(center);
    }
    for (const entity of this.frame.entities) if (entity.contextHash !== ZERO_CONTEXT_HEX) { const legacy=new THREE.Color().setHSL(renderTraits(entity.hash, entity.provenance, entity.contextHash, true).accentHue, numberValue(this.visual,'context.colorInfluence'), 0.6);const ring = new THREE.Mesh(new THREE.TorusGeometry(0.34*numberValue(this.visual,'context.scale'), numberValue(this.visual,'context.thickness'), 6, numberValue(this.visual,'context.segments')), new THREE.MeshBasicMaterial({ color: this.color('Context',entity.contextHash,`#${legacy.getHexString()}`,'context/ring'), transparent: true, opacity: numberValue(this.visual,'context.opacity') })); ring.position.copy(positions.get(entity.hash)!);ring.visible=booleanValue(this.visual,'context.enabled'); this.groups.contexts.add(ring); }
    for (const record of this.frame.condensationRecords) {
      const child = positions.get(record.entityHash); if (!child) continue;
      const accent = new THREE.Mesh(new THREE.SphereGeometry(0.48, 12, 8), new THREE.MeshBasicMaterial({ color: this.color('Condensed Entity',record.entityHash,stringValue(this.visual,'condensation.color'),'condensed/accent'), wireframe: true, transparent: true, opacity: 0.45 })); accent.position.copy(child); this.groups['condensed-entities'].add(accent);
      for (const parent of record.parentHashes) if (positions.has(parent)) this.groups.ancestry.add(cylinderBetween(child, positions.get(parent)!, 0.008, new THREE.MeshBasicMaterial({ color: this.color('Accent',parent,'#ffd37a','ancestry/line'), transparent: true, opacity: 0.18 })));
    }
  ...
  private rebuildSelection(): void {
    ...
    const selectedEntity=this.frame.entities.find((entity)=>entity.hash===this.selected)!;const vfxIntensity=vfxTargetMatches(stringValue(this.visual,'vfx.selective.target'),'Selected Entity')?selectiveBloomIntensity(this.visual,{entityHash:selectedEntity.hash,contextHash:selectedEntity.contextHash,dimension:'dimension-0'}):0;const highlight = new THREE.Mesh(new THREE.SphereGeometry(0.55*numberValue(this.visual,'selection.scale'), 16, 10), new THREE.MeshBasicMaterial({ color: new THREE.Color(this.color('Selection',selectedEntity.hash,stringValue(this.visual,'selection.color'),'entity/selection')).multiplyScalar(1+vfxIntensity), wireframe: true, transparent: true, opacity: numberValue(this.visual,'selection.opacity'),toneMapped:vfxIntensity===0 }));
